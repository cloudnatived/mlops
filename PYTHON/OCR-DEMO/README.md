...
读光OCR系列模型中涉及的ConvNextViT模型，主要包括三个主要部分，Convolutional Backbone提取图像视觉特征，
ConvTransformer Blocks用于对视觉特征进行上下文建模，最后连接CTC loss进行识别解码以及网络梯度优化。识别模型结构如下图
...
然后，OCR文字识别这个应用场景下，没有GPU也是OK的，但如果你装了GPU版本的tensorflow代码会默认在GPU上运行。
这些依赖都搞定以后，咱们就可以通过下面这部分的代码自验，并且开始下载对应的ConvNextViT模型了
#################################
