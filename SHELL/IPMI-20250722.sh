#!/bin/bash
#
# 作者：wangzhen2，2025-07-21，在ubuntu22.04.5上测试通过
...
process_ip() {
    ...
    # 判断IP地址的IPMI端口623是否可访问。目前测试得出：telnet sugon、lenovo、IBM的服务器，是可以连接的。telnet huawei的服务器，没有返回，但是用ipmitool是正常返回的。
    #timeout 3 bash -c "</dev/tcp/$ip/623" 2>/dev/null
    timeout 3 bash -c "</dev/tcp/$ip/80" 2>/dev/null
    if [ $? -eq 0 ]; then
        mkdir -p ./IPMILOG/
