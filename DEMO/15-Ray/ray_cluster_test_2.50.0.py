...
class RayClusterTester:
    def __init__(self, address: str = None):
        ...
        Args:
            address: Ray集群头部节点地址，如"ray://172.18.6.60:10001"
                     若为None，则尝试连接本地Ray实例
        """
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            # Ray 2.50.0 推荐的初始化方式，适配新参数
            ray.init(
                address=self.address,
                ignore_reinit_error=True,
                logging_level=logging.WARNING,
                # 2.50.0 新增的推荐参数，避免弃用警告
                runtime_env={"env_vars": {"RAY_DISABLE_IMPORT_RESOLVE_WARNING": "1"}}
            )
    ...
    def get_cluster_info(self) -> Dict:
        ...
        try:
            # Ray 2.50.0 中部分API调整，兼容获取资源的方式
            resources = ray.cluster_resources()
            available_resources = ray.available_resources()
            
            # 获取节点信息（2.50.0中nodes()返回格式无变更，但增加容错处理）
            nodes = ray.nodes()
    ...
    def print_cluster_info(self):
        ...
        for i, node in enumerate(self.cluster_info['nodes']):
            print(f"  节点 {i+1}:")
            print(f"    节点ID: {node.get('NodeID', 'N/A')[:8]}...")
            print(f"    地址: {node.get('NodeManagerAddress', 'N/A')}:{node.get('NodeManagerPort', 'N/A')}")
            print(f"    资源: {node.get('Resources', {})}")
            print(f"    状态: {node.get('Alive', 'N/A')}")

    @ray.remote
    def _test_task(self, x: int) -> int:  # 修正：实例方法需传入self
        """测试任务函数，用于验证基本任务执行（适配Ray 2.50.0）"""
        import time
        import os
    ...
    def test_basic_tasks(self, num_tasks: int = 10) -> bool:
        ...
        try:
            ...
            # 提交多个任务
            futures = [self._test_task.remote(self, i) for i in range(num_tasks)]  # 修正：传入self
    ...
    @ray.remote(num_gpus=0.1)  # Ray 2.50.0 中GPU资源申请方式兼容
    def _gpu_test_task(self) -> Dict:  # 修正：实例方法需传入self
        """GPU测试任务，检查GPU资源是否可用（适配Ray 2.50.0）"""
        import torch
        import os
    ...
    def test_gpu_resources(self, num_tasks: int = 2) -> bool:
        ...
        try:
            ...
            # 提交GPU测试任务（修正：传入self）
            futures = [self._gpu_test_task.remote(self) for _ in range(num_tasks)]
            results = ray.get(futures)
    ...
    @ray.remote
    def _distributed_task(self, idx: int, shared_queue: ray.Queue) -> int:  # 修正：实例方法需传入self
        """分布式任务，用于测试节点间通信（适配Ray 2.50.0 Queue API）"""
        import time
        ...
        while not shared_queue.empty() and messages_received < 3:
            try:
                msg = shared_queue.get(timeout=0.5)  # 2.50.0 推荐添加超时，避免阻塞
                messages_received += 1
    ...
    def test_distributed_communication(self, num_tasks: int = 5) -> bool:
        ...
        try:
            ...
            # 创建共享队列（Ray 2.50.0 中Queue初始化兼容）
            shared_queue = ray.Queue(maxsize=100)
            
            # 提交分布式任务（修正：传入self）
            futures = [self._distributed_task.remote(self, i, shared_queue) for i in range(num_tasks)]
            results = ray.get(futures)
    ...
    @ray.remote
    def _fault_tolerance_task(self, task_id: int, fail_probability: float = 0.3) -> int:  # 修正：实例方法需传入self
        """容错测试任务，有一定概率失败"""
        import time
        time.sleep(0.2)
    ...
    def test_fault_tolerance(self, num_tasks: int = 10) -> bool:
        ...
        try:
            ...
            # 提交可能失败的任务（修正：传入self）
            futures = [self._fault_tolerance_task.remote(self, i) for i in range(num_tasks)]
    ...
    def test_large_data_transfer(self, data_size_mb: int = 10) -> bool:
        ...
        try:
            ...
            # 将数据发送到远程任务并返回（Ray 2.50.0 中推荐使用fn.remote而非嵌套定义）
            def process_large_data(data):
                return len(data)
            
            # 执行任务
            remote_func = ray.remote(process_large_data)
            future = remote_func.remote(data)
            result = ray.get(future)
    ...
    def shutdown(self):
        """关闭Ray连接（Ray 2.50.0 中shutdown API兼容）"""
        if self.connected:
            logger.info("关闭Ray连接")
            ray.shutdown()
            self.connected = False
...
if __name__ == "__main__":
    # 解析命令行参数获取Ray集群地址
    import argparse
    parser = argparse.ArgumentParser(description='Ray集群测试工具（适配Ray 2.50.0）')
    parser.add_argument('--address', type=str, default=None, 
                      help='Ray集群头部节点地址，如"ray://172.18.6.60:10001"')
